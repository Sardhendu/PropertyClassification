...
def vae_loss(x, x_decoded_mean_squash, z_mean, z_log_var, input_dimension):
    ...
    with tf.name_scope("cross_entropy"):
        ...
        print ('observedobserved ', observed.shape)
        # xent_loss = -tf.reduce_sum(K.flatten(x) * tf.log(observed) + (1 - K.flatten(x)) * tf.log(1 - observed), 1)
        # print ('sdadasdasdsa ', xent_loss.shape)
        xent_loss = input_dimension * metrics.binary_crossentropy(
                    K.flatten(x),
                    K.flatten(observed))
        
        print ('xent_lossxent_loss ', xent_loss.shape)
        # b_xent_loss = input_dimension * ops.get_loss(y_true=x, y_logits=x_decoded_mean,
        # which_loss='sigmoid_cross_entropy', lamda=None)  # Entropy loss of input and
        # # b_xent_loss = metrics.binary_crossentropy(x, x_decoded_mean)
        # print('12121212121212', b_xent_loss.shape)
    ...
    return vae_loss, tf.reduce_mean(xent_loss), tf.reduce_mean(kl_loss)
...
# z_mean.shape:  (?, 64)
# z_log_var.shape:  (?, 64)
# Decoder: fc1.shape:  (?, 512)
# Decoder: fc2.shape:  (?, 2048)
# Decoder: rsp_imp.shape:  (?, 8, 8, 32)
# Decoder: dc1.shape:  (?, ?, ?, 64)
# Decoder: dc2.shape:  (?, ?, ?, 64)
# Decoder: dc3.shape:  (?, ?, ?, 64)
# Decoder: dc4.shape:  (?, ?, ?, 3)
