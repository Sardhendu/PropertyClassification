...
from config import myNet, pathDict
from conv_net.resnet import mixture_of_experts
from conv_net.ops import summary_builder
from conv_net.train import load_batch_data
from conv_net.train import PropertyClassification
...
m2_batch_dir = os.path.join(pathDict['general_batch_path'], 'overlayed')
...
class MixtureModels(PropertyClassification):
    ...
    def cvalid(self, sess):
        len_cv_data = self.cvbatchX_m1.shape[0]
        ...
        for ite in range(0, self.cv_num_batches):
            ...
            cvX_m1 = self.cvbatchX_m1[from_idx: to_idx, :]
            cvY_m1 = self.cvbatchY_1hot_m1[from_idx: to_idx, :]

            cvX_m2 = self.cvbatchX_m2[from_idx: to_idx, :]
            cvY_m2 = self.cvbatchY_1hot_m2[from_idx: to_idx, :]

            logging.info('Running Cross Validation batch%s: cvX_m1.shape = %s, cvY_m1.shape = %s cvX_m2.shape = %s, '
                         'cvY_m2.shape = %s', str(ite), str(cvX_m1.shape), str(cvY_m1.shape), str(cvX_m2.shape), str(cvY_m2.shape))

            feed_dict = {
                self.computation_graph['inpX1']: cvX_m1,
                self.computation_graph['inpX2']: cvX_m1,
            ...
            cv_recall_score = Score.recall(self.cvbatchY_m1[from_idx: to_idx], cv_pred, reverse=True)
            cv_precision_score = Score.precision(self.cvbatchY_m1[from_idx: to_idx], cv_pred, reverse=True)

            # We do the below to produce the average
            inter_loss += cv_loss * len(cvX_m1)
            inter_acc += cv_acc * len(cvX_m1)
            inter_recall += cv_recall_score * len(cvX_m1)
            inter_precision += cv_precision_score * len(cvX_m1)
            tot_len += len(cvX_m1)
    ...
    def run_epoch(self, get_stats_at):
        ...
        with tf.Session(config=config_) as sess:
            ...
            self.cvbatchX_m1, self.cvbatchY_1hot_m1, self.cvbatchX_m2, self.cvbatchY_1hot_m2 = self.get_cvalid_data(sess)
