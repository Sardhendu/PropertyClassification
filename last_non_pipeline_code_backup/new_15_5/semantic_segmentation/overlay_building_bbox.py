...
def get_overlayed_image(parcels_in_image, tl_pxl, img, zoom=19, map_size=[400,400]):
    '''
...
def overlay_parcel_on_images(data_to_model, aerial_img_path, overlayed_img_path, zoom, map_size):
    ...
    for sc_cnt,scoop in enumerate(scoop_arr):
        ...
        ################ Step 2.
        property_parcel = gpd.read_file(os.path.join(pathDict['chicago_bbox_shp_files'], lat_scoop[scoop]))
        ...
        for rcnt, (pin, lat, lon, label) in enumerate(dataIN):
            ...
            if label == 'Likely Land':
                ...
                overlayed_dump_path = os.path.join(overlayed_land_path, '%s.jpg'%str(pin))
                ...
                overlayed_dump_path = os.path.join(overlayed_house_path, '%s.jpg' % str(pin))
            ...
            if len(parcels_in_image) >0:
                overlayed_image = get_overlayed_image(parcels_in_image, tl_pxl, img, zoom, map_size)
            else:
                overlayed_image = img
