...
class Preprocessing():
    ...
    def __init__(self, conf, pprocessor_inp_img_shape, pprocessor_inp_crop_shape, model_inp_img_shape):
        '''
    ...
    def random_rotate_image(self,image):
        if self.conf['preprocess_seed_idx'] == len(self.conf['seed_arr']) - 1:
            self.conf['preprocess_seed_idx'] = 0
    
        np.random.seed(self.conf['preprocess_seed_idx'])
    ...
    def randomCrop(self, imageIN):
        ...
        return tf.random_crop(imageIN, self.pprocessor_inp_crop_shape,
                              seed=self.conf['seed_arr'][self.conf['preprocess_seed_idx']])
    ...
    def randomVFlip(self, imageIN):
        ...
        if self.conf['preprocess_seed_idx'] == len(self.conf['seed_arr']) - 1:
            self.conf['preprocess_seed_idx'] = 0
        # Given an image this operation may or may not flip the image
        return tf.image.random_flip_up_down(imageIN, seed=self.conf['seed_arr'][self.conf['preprocess_seed_idx']])
    
    def randomRotate(self, imageIN):
        logging.info('Performing Random Rotation')
        # Given an image this operation may or may not flip the image
        return tf.py_func(self.random_rotate_image, [imageIN], tf.uint8)
