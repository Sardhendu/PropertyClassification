...
def vgg_fcn_addition(net, keep_prob, img_shape):
    ...
    logging.info('W6 shape = %s' % str(W6.shape))
    logging.info('b6 shape = %s' % str(b6.shape))
    ...
    logging.info('W7 shape = %s' % str(W7.shape))
    logging.info('b7 shape = %s' % str(b7.shape))
    ...
    logging.info('W_t1 shape = %s'%str(W_t1.shape))
    b_t1 = ops.bias_variable([deconv1_shape[3].value], name="b_t1")
    logging.info('b_t1 shape = %s' % str(b_t1.shape))
    logging.info('transposed on conv8 shape = %s', str(conv8.shape))
