...
def conv_layer(conf, X, k_shape, stride=1, padding='SAME',  w_init='tn', w_decay=None, scope_name='conv_layer', add_smry=True):
    # m, h, w, f = X.get_shape().as_list()
    
    if conf['weight_seed_idx'] == len(conf['seed_arr']) - 1:
        conf['weight_seed_idx'] = 0
    
    logging.info('SEED for scope: %s', str(conf['seed_arr'][conf['weight_seed_idx']]))
    
    if w_init == 'gu':
        wght_init = tf.glorot_uniform_initializer(seed=conf['seed_arr'][conf['weight_seed_idx']])
    else:
        wght_init = tf.truncated_normal_initializer(
                stddev=0.1, seed=conf['seed_arr'][conf['weight_seed_idx']]
        )
...
def conv2D_transposed_strided(conf, X, k_shape, stride=2, padding='SAME',  w_init='tn', out_shape=None, scope_name='conv_layer', add_smry=True):
    '''
    :param X:           The input
...
def batch_norm(conf, X, axis=[0,1,2], scope_name=None):
    '''
    ...
    decay = conf['myNet']['batch_norm_decay']
...
def fc_layers(conf, X, k_shape, w_init='tn', scope_name='fc_layer', add_smry=True):
    if conf['weight_seed_idx'] == len(conf['seed_arr']) - 1:
        conf['weight_seed_idx'] = 0
...
def optimize(conf, loss, learning_rate_decay=True, add_smry=True):
    print ('Learning Rate: Initial: ', conf['myNet']['learning_rate'])
    globalStep = tf.Variable(0, dtype=tf.float32)
    if learning_rate_decay:
        
        l_rate = tf.train.exponential_decay(conf['myNet']['learning_rate'],
                                                  globalStep * conf['myNet']['batch_size'],  # Used for decay computation
                                                  conf['myNet']['lr_decay_steps'],  # Decay steps,
                                                  conf['myNet']['learning_rate_decay_rate'],  # Decay rate
                                                  staircase=True)  # Will decay the learning rate in discrete interval
        
    else:
        l_rate = tf.constant(conf['myNet']['learning_rate'])
    
    # We would like to store the summary of the loss to watch the decrease in loss.

    logging.info('INITIALIZING OPTIMIZATION WITH %s: .....', str(conf['myNet']['optimizer']))
    with tf.name_scope("Optimizer"):
        if conf['myNet']['optimizer'] == 'ADAM':
            optimizer = tf.train.AdamOptimizer(learning_rate=l_rate).minimize(loss, global_step=globalStep)
        
        elif conf['myNet']['optimizer'] == 'RMSPROP':
            # keras.optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=None, decay=0.0)
            
            optimizer = tf.train.RMSPropOptimizer(learning_rate=l_rate, momentum=tf.constant(conf['myNet']['momentum'])).minimize(
