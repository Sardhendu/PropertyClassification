...
from src.conv_net.train import PropertyClassification, load_batch_data
...
class Test(PropertyClassification):
    ...
    def _init__(self, conf, params, device_type, which_net):
        PropertyClassification.__init__(self, conf, params, device_type, which_net)

    
    def run_model(self, batchX, batchY, sess):
        ...
        # We use reverse as true because, the business case is to identify more lands than houses and lands are
    ...
    def test(self,  checkpoint_path):
        ...
        with tf.Session(config = config_) as sess:
            ...
            if self.batch_name:
                ...
                batch_name_arr = [dirs.split('.')[0] for dirs in os.listdir(self.conf['pathDict']['batch_path']) if
                                  dirs!='.DS_Store']
                
            print('Batch path %s, batch_names: %s'%(str(self.conf['pathDict']['batch_path']), str(batch_name_arr)))
            ...
            for batch_num, batch_name in enumerate(batch_name_arr):
                batchX, batchY = load_batch_data(self.conf, which_data=batch_name)
                out_prob, tst_loss, tst_acc, ts_precsion_score, ts_recall_score = self.run_model(batchX, batchY, sess)
    ...
    def run(self, use_checkpoint_for_run, use_checkpoint_for_imageType, optional_batch_name=None,
        ...
        self.stats_path = os.path.join(self.conf['pathDict']['statistics_path'], 'prediction_stats')
        # Override the checkpoint path.
        self.ckpt_path = os.path.join(self.conf['pathDict']['parent_checkpoint_path'], use_checkpoint_for_run,
        ...
        for path_num, chk_path in enumerate(checkpoint_paths):
            ...
            if self.which_net == 'resnet':
                ...
                self.computation_graph = resnet(self.conf, img_shape=self.model_inp_img_shape, device_type=self.device_type,
                                                use_dropout=False)
