...
# from src.conv_net.conv_autoencoder import conv_autoencoder
from src.conv_net.convnet import conv_net
from src.conv_net.ops import summary_builder
from src.conv_net.resnet import resnet
from src.conv_net.utils import Score
from src.conv_net.utils import unison_shuffled_copies
from src.data_transformation.data_io import getH5File
...
def load_batch_data(conf, which_data='cvalid', force_dir_fetch=None):
    if force_dir_fetch:
        data_path =force_dir_fetch
    else:
        data_path = conf['pathDict']['batch_path']  # % (str(image_type))]
...
class PropertyClassification(object):
    def __init__(self, conf, params, device_type, which_net):
        self.conf = conf
        ...
        if 'learning_rate' in params_keys:
            conf['myNet']['learning_rate'] = params['learning_rate']
        ...
        if self.save_checkpoint or self.use_checkpoint:
            self.ckpt_path = os.path.join(conf['pathDict']['checkpoint_path'], self.which_net)
            if not os.path.exists(self.ckpt_path):
                os.makedirs(self.ckpt_path)
            print('Dumping/Retreiving Checkpoints to/from %s', self.ckpt_path)
        
        if self.write_tensorboard_summary:
            self.smry_path = os.path.join(conf['pathDict']['summary_path'], self.which_net)
...
class Train(PropertyClassification):
    def _init__(self, conf, params, device_type, which_net):
    ...
    def run_epoch(self, get_stats_at):
        ...
        with tf.Session(config=config_) as sess:
            ...
            cvbatchX, self.cvbatchY = load_batch_data(self.conf, which_data='cvalid')
            ...
            for epoch in range(self.max_epoch, self.max_epoch + self.epochs):
                ...
                for batch_num in range(self.max_batch, self.num_batches):
                    ...
                    batchX, batchY = load_batch_data(self.conf, which_data='batch_%s' % (batch_num))
    ...
    def run(self, num_epochs, num_batches, cv_num_batches, get_stats_at=10):
        ...
        if self.which_net == 'resnet':
            self.computation_graph = resnet(self.conf, img_shape=self.model_inp_img_shape, device_type=self.device_type, use_dropout=True)
        elif self.which_net == 'convnet':
            self.computation_graph = conv_net(self.conf, img_shape=self.model_inp_img_shape, device_type=self.device_type)
        else:
...
#                 for batch_num in range(self.max_batch, self.num_batches):
#                     self.batch_num = batch_num
#                     batchX, batchY = load_batch_data(self.conf, which_data='train_%s' % (batch_num))
#
...
# #                               image_type='assessor_code')
# #         (tr_loss_arr, cv_loss_arr, l_rate_arr, cv_reconstruction_mse,
# #          cv_reconstruction_entropy, cvY_label) = tr_obj.run(num_epochs=100, num_batches=max_batches + 1,
# # get_stats_at=3,
# #                                                             plot=True)  # + 1)
# #         print('')
# #         print('')
