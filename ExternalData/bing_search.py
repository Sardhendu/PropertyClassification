...
loc_tail = '?o=json&key=' + config.bing_key

aerial_url = 'https://dev.virtualearth.net/REST/v1/Imagery/Metadata/Aerial/'
aer_tail = '&o=json&key=' + config.bing_key
...
class BingSearch():
    ...
    def get_aerial_image(self, lat, lon):
        ...
        zoom_lvl = 21   # The maximum zoom level provided Bing Map
...
def get_image_address(dataIN, batch_size):
    ...
    for num, (pin, add1, city, zip, indicator) in enumerate(data_arr):
        ...
        if str(add1) != 'nan':
            ...
            lat, lon, ll_url = obj_BS.get_latlon()
            ...
            if lt != 'nan':
                ...
                if img == 'yes':
                    if indicator == "Likely House":
                        with open(os.path.join(pathDict['aerial_image_path'], 'house', '%s.jpg'%str(pin)), 'wb') as handler:
                            handler.write(image_data)
                    elif indicator == 'Likely Land':
                        with open(os.path.join(pathDict['aerial_image_path'], 'land', '%s.jpg'%str(pin)), 'wb') as handler:
                            handler.write(image_data)
                    else:
                        with open(os.path.join(pathDict['aerial_image_path'], 'unknown', '%s.jpg' % str(pin)), 'wb') as handler:
                            handler.write(image_data)
        
        statistics.append([p,cty,zp,add,lt,ln,zl,img,ll_url,img_url])
        

        
        if ((num+1)%batch_size) == 0 or num ==len(data_arr)-1:
            path = os.path.join(pathDict['statistics_path'],
                                'aerial_images_stats',
                                '%s_%s.csv'%(prev, num))
            statistics = pd.DataFrame(statistics,
                                      columns=['pin', 'city', 'zip', 'address',
                                               'lat', 'lon','zoom_lvl', 'image_fetch',
                                               'lat_lon_url', 'img_url'])
            statistics.to_csv(path, index=None)
            prev = num + 1
